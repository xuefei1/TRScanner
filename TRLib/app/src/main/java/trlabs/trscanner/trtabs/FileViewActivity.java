...
import trlabs.trscanner.trtabs.File.FileExplorerPreferenceActivity;
import trlabs.trscanner.trtabs.File.FileIconHelper;
import trlabs.trscanner.trtabs.File.FileInfo;
import trlabs.trscanner.trtabs.adapter.FileListAdapter;
...
import trlabs.trscanner.trtabs.config.Settings;
import trlabs.trscanner.ui.PullToRefreshListView;

public class FileViewActivity extends Fragment implements
        IFileInteractionListener, TRScannerTabActivity.IBackPressedListener {
    ...
    private ArrayList<FileInfo> mFileNameList = new ArrayList<FileInfo>();  // find all files with FileInfo types
    ...
    private View lvFiles_footer;
    private TextView lvFiles_foot_more;
    private ProgressBar lvFiles_foot_progress;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        /*
        View layout = inflater.inflate(R.layout.file_explorer_list, container, false);
        lvFiles_footer = inflater.inflate(R.layout.listview_footer, null);
        lvFiles_foot_more = (TextView) lvFiles_footer.findViewById(R.id.listview_foot_more);
        lvFiles_foot_progress = (ProgressBar) lvFiles_footer.findViewById(R.id.listview_foot_progress);

        lvFiles = (PullToRefreshListView) layout.findViewById(R.id.file_refreshlist);
        lvFiles.addFooterView(lvFiles_footer);
        lvFiles.setOnScrollListener(new AbsListView.OnScrollListener() {
            @Override
            public void onScrollStateChanged(AbsListView absListView, int i) {
                if (FileTools.isEmptyDirectory(GlobalConsts.ROOT_PATH))
                    return;
        ...
        */
        ...
        if (!TextUtils.isEmpty(action)
            ...
            if (!pickFolder) {
                ...
                mFileCategoryHelper.setCustomCategory(new String[]{} /*folder only*/);
        ...
        String rootDir = GlobalConsts.ROOT_PATH;
        ...
        */
        ...
        String currentDir = GlobalConsts.ROOT_PATH;
        ...
        mBackspaceExit =
                (uri != null)                          && (TextUtils.isEmpty(action)
                || (!action.equals(Intent.ACTION_PICK) && !action.equals(Intent.ACTION_GET_CONTENT)));
        ...
        for (File child : listFiles) {
            ...
            if (Util.isNormalFile(absolutePath) && Util.shouldShowFile(absolutePath)) {
                FileInfo lFileInfo = Util.GetFileInfo(child,
                        mFileCategoryHelper.getFilter(), Settings.instance().getShowDotAndHiddenFiles());
                if (lFileInfo != null) {
